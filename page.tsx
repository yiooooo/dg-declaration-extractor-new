...
export default function App() {
  ...
  useEffect(() => {
    ...
    const savedOpenaiBaseUrl = localStorage.getItem('dg_openai_base_url');
    if (savedOpenaiBaseUrl) setOpenaiBaseUrl(savedOpenaiBaseUrl);

    const savedGeminiBaseUrl = localStorage.getItem('dg_gemini_base_url');
    if (savedGeminiBaseUrl) setGeminiBaseUrl(savedGeminiBaseUrl);
  }, []);

  useEffect(() => {
    if (provider) localStorage.setItem('dg_provider', provider);
    if (apiKey) localStorage.setItem('dg_api_key', apiKey);
    localStorage.setItem('dg_openai_base_url', openaiBaseUrl);
    localStorage.setItem('dg_gemini_base_url', geminiBaseUrl);
  }, [provider, apiKey, openaiBaseUrl, geminiBaseUrl]);

  useEffect(() => {
    // Expose to window for emergency access
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-[50]">
          ...
          <div className="flex gap-3 relative z-[60]">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-white border-2 border-gray-200 text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all active:scale-95 shadow-sm relative z-[1000]"
              title="API Settings"
            >
              <Settings className="w-5 h-5" />
              <span className="font-medium">API 設定</span>
            </button>
            <button
              type="button"
        ...
        <div className="relative w-full max-w-md bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] p-8 overflow-hidden border border-gray-100">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input
                type="text"
                value={provider === 'openai' ? openaiBaseUrl : geminiBaseUrl}
                onChange={(e) => {
                  if (provider === 'openai') setOpenaiBaseUrl(e.target.value);
                  else setGeminiBaseUrl(e.target.value);
                }}
